...
object Main {
  ...
  val moActionInner = {
    ...
    query.sortBy((tuple: (ArtistTable, AlbumTable)) => tuple._1.name).result
  }
  ...
  val moActionExplicitInner = {
    val query = ArtistTable.join(AlbumTable).
      on((artist: ArtistTable, album: AlbumTable) => artist.id === album.artistId)
    query.sortBy((tuple: (ArtistTable, AlbumTable)) => tuple._1.name).result
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("*" * 100)
    exec(moActionExplicitInner.transactionally).foreach(println)
  }
